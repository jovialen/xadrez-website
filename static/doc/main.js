...
(function () {
	...
	function handleHashes(ev) {
		...
		if (savedHash !== window.location.hash) {
			...
			expandSection(savedHash.slice(1));
		}
	}
	...
	window.addEventListener('resize', () => {
		if (window.innerWidth > window.RUSTDOC_MOBILE_BREAKPOINT && oldSidebarScrollPosition !== null) {
			hideSidebar();
		}
	});
	...
	onEachLazy(document.getElementsByClassName('notable-traits'), (e) => {
		e.onclick = function () {
			this.getElementsByClassName('notable-traits-tooltiptext')[0].classList.toggle(
				'force-tooltip'
			);
		};
	});
