...
(function () {
	...
	function initSearch(rawSearchIndex) {
		...
		function getIdentEndPosition(parserState) {
			...
			while (parserState.pos < parserState.length) {
				...
				if (!isIdentCharacter(c)) {
					...
					} else if (isStopCharacter(c) || isSpecialStartCharacter(c) || isSeparatorCharacter(c)) {
						break;
					} else if (c === ':') {
		...
		function getNextElem(query, parserState, elems, isInGenerics) {
			...
			if (parserState.pos < parserState.length && parserState.userQuery[parserState.pos] === '<') {
				if (isInGenerics) {
		...
		function execQuery(parsedQuery, searchWords, filterCrates, currentCrate) {
				...
				if (literalSearch) {
					if (lev !== 0) {
						if (elem.generics.length === 0) {
							...
							if (
								checkGeneric &&
								row.generics.findIndex((tmp_elem) => tmp_elem.name === elem.name) !== -1
							) {
								return 0;
							}
						}
			...
			function handleSingleArg(row, pos, elem, results_others, results_in_args, results_returned) {
				...
				if (
					searchWord.indexOf(elem.pathLast) > -1 ||
		...
		function buildHrefAndPath(item) {
			...
			if (type === 'mod') {
				displayPath = path + '::';
				href = ROOT_PATH + path.replace(/::/g, '/') + '/' + name + '/index.html';
			} else if (type === 'import') {
				displayPath = item.path + '::';
				href = ROOT_PATH + item.path.replace(/::/g, '/') + '/index.html#reexport.' + name;
			} else if (type === 'primitive' || type === 'keyword') {
				displayPath = '';
				href = ROOT_PATH + path.replace(/::/g, '/') + '/' + type + '.' + name + '.html';
				...
				href =
					ROOT_PATH + path.replace(/::/g, '/') + '/' + pageType + '.' + pageName + '.html' + anchor;
			} else {
				displayPath = item.path + '::';
				href = ROOT_PATH + item.path.replace(/::/g, '/') + '/' + type + '.' + name + '.html';
			}
		...
		function showResults(results, go_to_first, filterCrates) {
			...
			if (crates_list.length > 1) {
				crates =
					' in&nbsp;<div id="crate-search-div"><select id="crate-search">' +
			...
			if (results.query.error !== null) {
				...
				output += '<div id="titles">' + makeTabHeader(0, 'In Names', ret_others[1]) + '</div>';
				currentTab = 0;
				...
				output +=
					'<div id="titles">' + makeTabHeader(0, signatureTabTitle, ret_others[1]) + '</div>';
				currentTab = 0;
			}
		...
		function putBackSearch() {
			...
			if (search_input.value !== '' && !searchState.isDisplayed()) {
				...
				if (browserSupportsHistoryApi()) {
					history.replaceState(null, '', buildUrl(search_input.value, getFilterCrates()));
				}
